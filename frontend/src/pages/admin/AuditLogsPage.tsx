...
const AuditLogsPage = () => {
  ...
  return (
    <div>
      ...
      {loading ? <LoadingSpinner /> : logs.length === 0 ? <EmptyState title="Nenhum log" icon={<FileText className="w-8 h-8 text-muted-foreground/80" />} /> : (
        <>
          <div className="bg-card/50 border border-border rounded-lg overflow-hidden">
            <Table><TableHeader><TableRow className="border-border hover:bg-transparent">
              <TableHead className="text-muted-foreground font-tactical">Data/Hora</TableHead>
              <TableHead className="text-muted-foreground font-tactical">Acao</TableHead>
              <TableHead className="text-muted-foreground font-tactical">Entidade</TableHead>
              <TableHead className="text-muted-foreground font-tactical">Descricao</TableHead>
              <TableHead className="text-muted-foreground font-tactical">Por</TableHead>
              <TableHead className="text-muted-foreground font-tactical">Det.</TableHead>
            </TableRow></TableHeader>
            <TableBody>{logs.map((log: any) => (
              <TableRow key={log.id} className="border-border">
                <TableCell className="text-muted-foreground font-tactical text-xs whitespace-nowrap">{formatDateTime(log.createdAt)}</TableCell>
                <TableCell><Badge variant="outline" className={`font-tactical text-xs ${actionColors[log.action] || 'border-input text-muted-foreground'}`}>{log.action}</Badge></TableCell>
                <TableCell className="text-foreground/85 font-tactical text-sm">{log.entityType}</TableCell>
                <TableCell className="text-foreground font-tactical text-sm max-w-xs truncate">{log.description}</TableCell>
                <TableCell className="text-muted-foreground font-tactical text-sm">{log.performedBy?.fullName}</TableCell>
          ...
          <div className="flex items-center justify-between mt-4">
            <Button variant="ghost" size="sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="font-tactical text-muted-foreground"><ChevronLeft size={16} className="mr-1" />Anterior</Button>
            <span className="text-muted-foreground/80 font-tactical text-sm">Pagina {page}/{totalPages}</span>
            <Button variant="ghost" size="sm" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="font-tactical text-muted-foreground">Proximo<ChevronRight size={16} className="ml-1" /></Button>
          </div>
        </>
      )}
          ...
          {selectedLog && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm font-tactical">
                <div><span className="text-muted-foreground/80">Acao:</span> <span className="text-foreground">{selectedLog.action}</span></div>
                <div><span className="text-muted-foreground/80">Entidade:</span> <span className="text-foreground">{selectedLog.entityType}</span></div>
                <div><span className="text-muted-foreground/80">Data:</span> <span className="text-foreground">{formatDateTime(selectedLog.createdAt)}</span></div>
                <div><span className="text-muted-foreground/80">Por:</span> <span className="text-foreground">{selectedLog.performedBy?.fullName}</span></div>
              </div>
