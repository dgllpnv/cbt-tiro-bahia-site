...
const KpiCard = ({
  ...
  return (
    <div
      className={`relative overflow-hidden bg-gray-900/50 border ${borderColor} rounded-lg p-4 group hover:border-opacity-80 transition-all duration-300`}
    >
      <div
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-1.5 min-w-0 flex-1">
          <p className="text-[10px] font-tactical uppercase tracking-wider text-gray-500 truncate">
            {label}
          </p>
          <p className="text-xl sm:text-2xl font-military font-bold text-white tracking-wide truncate">
            {value}
          </p>
          {hint && (
            <p className="text-[10px] font-tactical text-gray-500 truncate" title={hint}>
              {hint}
            </p>
          )}
          {hasDelta && (
          ...
          {!hasDelta && !hint && (
            <p className="text-[10px] font-tactical text-gray-600">—</p>
          )}
        </div>
