// DSA2 — Declaracao de Seguranca do Acervo (Anexo A) — segundo endereco
import {
...
export function buildDSA2Pdf(packet: DeclarationDataPacket) {
  const { member } = packet;
  ...
  const text =
    ...
    `residindo em nº ___, bairro ___________, cep: __________, ____/__ e CPF nº ${formatCpf(member.cpf)}. ` +
    `DECLARO, para fim de ATIRADOR DESPORTIVO, que o segundo local de guarda do meu acervo possui cofre ou ` +
    `lugar seguro, com tranca, para armazenamento das armas de fogo desmuniciadas de que sou proprietário, ` +
    `e de que adotarei as medidas necessárias para impedir que menor de dezoito anos de idade ou pessoa civilmente ` +
    `incapaz se apodere de arma de fogo sob minha posse ou de minha propriedade, observado o disposto no art. 13 ` +
    `da Lei nº 10.826, de 2003.`;

  declParagraph(ctx, text, { fontSize: 11 });

  // Primeiro endereco (do cadastro do associado)
  ctx.cursorY += 4;
  const primeiro = [
    member.address,
    member.neighborhood,
    ...
    .filter(Boolean)
    .join(', ') || '—';
  declParagraph(ctx, `PRIMEIRO ENDEREÇO: ${primeiro}`, {
    fontSize: 10,
    bold: true,
    align: 'left',
    indent: false,
